...
try {
	staticEnv = require(process.cwd() + path.sep + 'env.json');
} catch (e) {
	staticEnv = {};
}
...
function _loadConfigFile(file, config, ns) {
	if (file._ns === '' || !ns) {
		return utils.extend(config || {}, JSON.parse(fs.readFileSync(file._path, 'utf8')));
	}

	var obj = {};
	obj[file._ns] = JSON.parse(fs.readFileSync(file._path, 'utf8'));
	if (ns === 2) {
		obj = {'ns': obj};
	}
...
function _addFunctions(config) {
	...
	Object.defineProperty(config, 'writeConfigFile', {
      ...
      fs.writeFileSync(path.join(m._root, name), JSON.stringify(object, null, '	'));
	...
	Object.defineProperty(config, 'changeConfigFile', {
		value: function (name, object) {
			...
			var o = JSON.parse(fs.readFileSync(m[name]._path));
			object = utils.extend(o, object);
			fs.writeFileSync(m[name]._path, JSON.stringify(object, null, '	'));
