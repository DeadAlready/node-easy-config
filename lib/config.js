...
function _loadCommandLine(config) {
    ...
    cmdLine.forEach(function (val) {
        ...
        var index = tmp.shift();
        var value = utils.typeCast(tmp.shift());
 ...
 * @return {Object}
 */
function _loadEnvironment(config) {

    Object.keys(environment).filter(function (key) {
        return key.indexOf('NODE_') === 0 && key !== 'NODE_ENV';
    }).forEach(function (key) {
        var index = key.replace('NODE_', '').replace(/\_/g, '.');
...
function _loadConfig(options, force) {
    ...
    cmdLine.forEach(function (val) {
        if (val.charAt(0) === '-' && val.charAt(1) !== '-') {
            ...
            options[tmp[0]] = utils.typeCast(tmp[1]);
        }
    });
    ...
    if (options.envVars) {
        config = _loadEnvironment(config);
    }
